...
const getLineNumbers = line => line.filter(Number);

const getFirstNumber = line => {
  return getLineNumbers(line)[0];
}

const getLineCalibrationValue = line => {
  return getFirstNumber(line).concat(getFirstNumber(line.reverse()));
}
...
const reducer = (accumulator, line) => {
  console.log(accumulator, line, getLineTranslation(line));
  return Number(getLineCalibrationValue(getLineTranslation(line).split(''))) + accumulator;
};
