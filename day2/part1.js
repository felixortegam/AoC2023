...
const getGameValues = line => {
  return line.split(': ')[1]
    .replaceAll(';', ',')
    .split(', ')
    .map(values => {
      return values.split(' ')[0] > cubesQuantities[values.split(' ')[1]]
    });
}

const isValidGame = game => {
  return !getGameValues(game).includes(true);
};

const reducer = (accumulator, game) => {
  return isValidGame(game) ? accumulator + getGameID(game) : accumulator;
}
