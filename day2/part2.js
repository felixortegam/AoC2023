...
const getFewestCubes = game => {
  return getSetValues(game).reduce((accumulator, set) => {
    return Number(set[0]) > accumulator[set[1]]
      ? { ...accumulator, [set[1]]: Number(set[0]) }
      : { ...accumulator }
  }, { "red": 0, "green": 0, "blue": 0 })
}

const getGamePower = game => {
  const { red, green, blue } = getFewestCubes(game);

  return red * green * blue;
}

const reducer = (accumulator, game) => {
  return getGamePower(game) + accumulator;
};
